...
function(__glad_fortran_library FARGS FFILES)
    ...
    foreach(API ${GGF_API})
        __glad_extract_spec_profile_version(SPEC PROFILE VERSION "${API}")
        if(SPEC STREQUAL "gl")
            list(APPEND GGF_FILES
                "${GLAD_DIR}/include/glad/types.h"
                "${GLAD_DIR}/src/gl.F90"
                )
        else()
            message(FATAL_ERROR "Unknown SPEC: '${SPEC}'")
    ...
    endforeach()
    list(REMOVE_DUPLICATES GGF_FILES)

    set(GGF_ARGS "")

    set("${FARGS}" "${GGF_ARGS}" PARENT_SCOPE)
    set("${FFILES}" "${GGF_FILES}" PARENT_SCOPE)
endfunction()
...
function(glad_add_library TARGET)
    ...
    elseif(GLAD_LANGUAGE STREQUAL "fortran")
        __glad_fortran_library(LANG_ARGS GLAD_FILES ${GG_UNPARSED_ARGUMENTS} API ${GG_API})
    else()
